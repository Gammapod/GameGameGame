...
public sealed class ContentToolDispatcher(ContentToolSessionRegistry sessions)
{
    ...
    public ContentToolResponse Invoke(string toolName, object request)
    {
        try
        {
            return toolName switch
            {
                ...
                ContentToolNames.SetBehaviorStepTemplateId => WithSession((ContentToolSetBehaviorStepTemplateIdRequest)request, api => FromAgentResult(api, api.SetActionPlanBehaviorStepTemplateId(((ContentToolSetBehaviorStepTemplateIdRequest)request).ActionPlanTemplateId, ((ContentToolSetBehaviorStepTemplateIdRequest)request).StepIndex, ((ContentToolSetBehaviorStepTemplateIdRequest)request).TemplateId))),
                ContentToolNames.SetBehaviorStepCreatePlacement => WithSession((ContentToolSetBehaviorStepCreatePlacementRequest)request, api => FromAgentResult(api, api.SetActionPlanBehaviorStepCreatePlacement(((ContentToolSetBehaviorStepCreatePlacementRequest)request).ActionPlanTemplateId, ((ContentToolSetBehaviorStepCreatePlacementRequest)request).StepIndex, ((ContentToolSetBehaviorStepCreatePlacementRequest)request).CreatePlacement))),
                ContentToolNames.SetBehaviorStepTargetSelf => WithSession((ContentToolSetBehaviorStepTargetSelfRequest)request, api => FromAgentResult(api, api.SetActionPlanBehaviorStepTargetSelf(((ContentToolSetBehaviorStepTargetSelfRequest)request).ActionPlanTemplateId, ((ContentToolSetBehaviorStepTargetSelfRequest)request).StepIndex, ((ContentToolSetBehaviorStepTargetSelfRequest)request).TargetSelf))),
    ...
    [
        ...
        new("behavior-plan", "Create or edit a behavior-chain action plan", "Author normal action-plan behavior without legacy low-level YAML editing.",
            ["Create or inspect an action plan.", "List action steps to choose supported kinds.", "Set/add/reorder behavior steps.", "Use field-specific setters for target labels, targetSelf, direction/path/create options, template refs, plan refs, and costs.", "Preview the action plan."],
    ...
    private static ContentToolSchemaDescription DescribeSchema(string concept) => concept switch
    {
        ...
        "behaviorStep" => new(concept, "Behavior-chain Action Step kind plus optional fields set by field-specific tools.",
            [new("kind", "string", true, "Action Step kind.", Enum.GetNames<ActionPlanBehaviorStepKind>()), new("targetLabel", "string|null", false, "Stable labeled target reference.", NullableClears: true), new("targetSlot", "integer|null", false, "Compatibility numeric target slot.", NullableClears: true), new("targetSelf", "boolean", false, "Target the actor itself for steps such as PolymorphTarget."), new("templateId", "string|null", false, "Entity template reference for CreateEntity and PolymorphTarget.", NullableClears: true), new("createPlacement", "string|null", false, "CreateEntity placement mode.", Enum.GetNames<CreateEntityPlacement>(), NullableClears: true), new("counterpartyTargetLabel", "string|null", false, "Transfer-only stable labeled counterparty target reference.", NullableClears: true), new("counterpartyTargetSlot", "integer|null", false, "Transfer-only compatibility numeric counterparty target slot.", NullableClears: true), new("costs", "cost[]", false, "Optional required costs consumed by the step.")],
            ["{ \"kind\": \"Move\" }", "then call ggg_content_set_behavior_step_direction_mode for Move directionMode", "for CreateEntity call ggg_content_set_behavior_step_template_id and ggg_content_set_behavior_step_create_placement"]),
    ...
    private static object Deserialize(string toolName, JsonElement arguments) => toolName switch
    {
        ...
        ContentToolNames.SetBehaviorStepTemplateId => arguments.Deserialize<ContentToolSetBehaviorStepTemplateIdRequest>(JsonOptions)!,
        ContentToolNames.SetBehaviorStepCreatePlacement => arguments.Deserialize<ContentToolSetBehaviorStepCreatePlacementRequest>(JsonOptions)!,
        ContentToolNames.SetBehaviorStepTargetSelf => arguments.Deserialize<ContentToolSetBehaviorStepTargetSelfRequest>(JsonOptions)!,
