...
public sealed class AgentContentEditorApi(ContentEditorSession session, IAgentScenarioRecorder? scenarioRecorder = null)
{
    ...
    public AgentApiResult<AgentScenarioRecordingReport> RecordScenario(AgentScenarioRecordingRequest request) =>
        scenarioRecorder is null
            ? AgentApiResult<AgentScenarioRecordingReport>.Failure(new AgentApiError(
                "ScenarioRecordingUnavailable",
                "Scenario recording is implemented by headless tooling. Construct the API with an IAgentScenarioRecorder adapter to record scenarios.",
                Recoverable: true))
            : Try("RecordScenarioFailed", () => scenarioRecorder.Record(Session.Document, request));
    ...
    public AgentApiResult UpdateEntityTemplate(EntityTemplateId id, AgentEntityTemplateUpdate update) =>
        Try("UpdateEntityTemplateFailed", () =>
        {
            ...
            var template = preset.Template with
            {
                ...
                Material = update.ClearMaterial ? null : update.Material ?? preset.Template.Material
            };
