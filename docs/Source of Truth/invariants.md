...
- Runtime behavior-provider overrides are experimental spike behavior: when assigned, the actor uses the provider entity's action plan while preserving the actor as body/action-state owner, and the provider is not independently scheduled for that turn loop.
...
- Controlled actor commands for direct player/frontend input resolve through a shared Core service for move, pickup, drop, enter, and exit; successful commands consume/advance the turn, while failed commands record a trace and do not advance the turn.
- Experimental controlled overwrite commands for the runtime behavior-provider spike resolve through the same shared Core command service: GiveOverwrite assigns a carried provider to an adjacent target actor, and TakeOverwrite clears an adjacent target actor's provider while preserving/returning the provider to player inventory.
...
- Structured action outcome projections derive frontend log rows from structured command/action fields and preserve full traces for debug expansion.
- Entity panel projections combine inspection, containment path, action state, action-plan/behavior-provider summary, inventory grid, local turn-order contents, and structured local log snippets into frontend-neutral panel facts while leaving layout/focus/rendering frontend-owned.
...
- Runtime behavior-provider overrides: `ActorUsesBehaviorProviderDefaultPlanWhenOverrideIsAssigned`, `BehaviorProviderIsNotScheduledIndependentlyWhileAssigned`, `RemovingBehaviorProviderRestoresActorDefaultPlan`, `BehaviorOverridePreservesActorTargetingAndActionState`.
...
- Controlled actor command execution: `ControlledActorCommandMoveReturnsStructuredSuccessAndAdvancesTurn`, `ControlledActorCommandFailedMoveRecordsFailureWithoutAdvancingTurn`, `ControlledActorCommandPickupReportsTargetAndDestinationAnchors`.
- Experimental overwrite command execution: `ControlledActorCommandGiveOverwriteAssignsCarriedProviderToAdjacentTarget`, `ControlledActorCommandTakeOverwriteClearsProviderAndKeepsOrReturnsItToInventory`, `ControlledActorCommandGiveOverwriteFailsWhenProviderIsNotCarried`.
...
- Structured action outcome/log projection: `ActionOutcomeProjectionRendersSuccessfulPickupFromStructuredCommandResult`, `ActionOutcomeProjectionRendersFailedMoveWithoutParsingDisplaySummary`, `ActionLogProjectionFiltersOutcomesByEntityAndPlaneAnchors`.
- Entity panel projection: `EntityPanelProjectionCombinesIdentityPathStateGridAndContents`, `EntityPanelProjectionIncludesStructuredLocalLogSnippetsWhenAnchored`, `EntityPanelProjectionShowsBehaviorOverrideSource`, `EntityPanelProjectionContentsTreatBehaviorProviderAsInertAndOverrideActorAsActor`.
...
- Scenario catalog/selection contract: `ScenarioCatalogListsScenariosFromDocument`, `ScenarioCatalogDiscoversFolderAndRoundTripsManifest`, `PlayableScenarioLauncherBuildsFreshSessionFromCatalogEntry`, `ConsoleScenarioLauncherBuildsFreshSessionFromCatalogEntry`.
