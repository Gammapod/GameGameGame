...
| 2. Frontend state is presentation state. | Maintained `GameGameGame.Frontend.SadConsole.Tests` suites cover layout, selection, focus, prompt, animation, browser, gallery, and local presentation as pure frontend state. Historical legacy SadConsole component suites were removed after useful patterns were mined or logged. | Future hover, scrolling, mouse hit-testing, and animation behavior should add maintained SadConsole tests once stable enough to pin. |
| Inventory-space component remains presentation-only and layer-stable. | Maintained frontend grid/inspection/component-gallery tests cover renderer-neutral view models, backdrop/entity/decorator separation, current-space Play-mode consumption, required-size behavior, and executable gallery references where promoted. | Future true transparency, accent layers, semi-transparent information layers, scaling, scrolling, and mouse hit-testing should add focused tests before becoming durable patterns. |
| Consumer Play mode confines gameplay UI to drawable bounds. | Maintained frontend display/layout tests cover drawable bounds, scenario browser chrome, Play grid placement, and component placement inside resolved drawable regions. | Manual frontend smoke confirmed fullscreen and border visibility on multiple resolutions. Future display work should add evidence for pixel-perfect leftover-monitor-pixel centering if promoted from deferred UX spec note. |
...
- Only the SadConsole spike findings were carried forward to main. The old SadConsole prototype source is intentionally not a current codebase artifact. Current maintained SadConsole implementation lives in `src/GameGameGame.Frontend.SadConsole` and should continue consuming shared contracts rather than reviving prototype-local semantics.
...
- Several historical docs describe the completed prototype, removed legacy SadConsole project, and old run commands. Those commands are archival only. Main should carry forward the findings, not the removed project. Current SadConsole work should continue in `src/GameGameGame.Frontend.SadConsole`.
