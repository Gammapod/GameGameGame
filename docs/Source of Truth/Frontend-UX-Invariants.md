...
| 8. SadConsole is the preferred debug/editor browser direction, not a final engine lock. | `tests/GameGameGame.Frontend.SadConsole.Tests` is the automated frontend test home for the maintained SadConsole surface. `tests/GameGameGame.SadConsole.Tests` remains the legacy/reference SadConsole test home only for explicit opt-in mining outside default solution workflows. | Engine-selection, packaging, mouse comfort, animation, and final UI-framework decisions remain manual/research-backed until promoted into stable contracts. |
...
- Only the SadConsole spike findings were carried forward to main. The old `src/GameGameGame.SadConsolePrototype` prototype source is intentionally not a current codebase artifact. Current maintained SadConsole implementation lives in `src/GameGameGame.Frontend.SadConsole` and should continue consuming shared contracts rather than reviving prototype-local semantics.
...
- Several historical docs describe the completed prototype and old run commands. Those commands are archival only. Main should carry forward the findings, not the prototype project. Current SadConsole work should continue in `src/GameGameGame.Frontend.SadConsole`; `src/GameGameGame.SadConsole` is legacy/reference-only and kept outside default build/package workflows.
