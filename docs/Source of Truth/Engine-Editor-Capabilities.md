...
- frontend editor snapshots are available in Content through `FrontendEditorService`, a SadConsole-consumable service over `ContentEditorSession`/`ContentEditorService` that opens content files and exposes read-only scenario, entity-template/carried-layout, action-plan, validation diagnostic, YAML preview, diff, and turn-0 scenario preview DTOs without depending on Avalonia view models. `AgentContentEditorApi` remains a parallel editing surface over the same session/service layer.
...
| Targeting rules | Stable | Yes | Yes | Yes | Yes | Yes | Display | Template `targetingRules` select nearest same-plane matching target template in range before plan evaluation; rules may publish stable `label` keys. Behavior steps should prefer authored `targetLabel`, with numeric `targetSlot` retained for compatibility/advanced use. |
