...
- merged inventory topology layers are available as a prototype Core/Content/editor capability: authored `mergedLayers` compose one or more entity-owned inventory spaces into a durable navigable layer by explicit contribution origins, with optional cardinal edge-to-edge `seams` for same-index non-Euclidean links such as wraps and rotations, explicit `cellLinks`, and semantic aligned `joins` that resolve mismatched room/hallway edges into deterministic source-cell links. Content validation resolves owner entity IDs from authored carried layouts, rejects overlaps by default, allows explicit layout-only overlap via `allowLayoutOverlap: true` where contributor interiors remain walkable and cross-contributor movement is explicit-link/seam topology, rejects disconnected layers, duplicate/multi-layer owner participation, unusable/missing owners, seam edge-length mismatches, non-cardinal endpoints, invalid join spans/offsets, and directional conflicts before materialization. Editor services, agent snapshots/operations, frontend editor summaries, YAML round-tripping, scenario materialization, and Beta manual showcase scenarios can author/inspect the prototype shape. Polished frontend visualization, generated corridors/cells, true Möbius index/facing transforms, and final stable authoring UX remain follow-up.
...
- merged inventory layers are exposed through top-level Content YAML `mergedLayers`, editor service and agent API upsert/list/snapshot operations, and frontend editor summaries. Current authoring uses explicit `spaces[].owner` plus `spaces[].origin` contributions, optional `seams[].first/second` cardinal edge endpoints, optional lower-level `cellLinks`, semantic aligned `joins`, and optional `allowLayoutOverlap: true` for explicit-link/seam-only overlapping layout experiments; this remains a prototype topology spike while generated placement and polished human-facing visualization are deferred.
