...
- headless scenario run reports now use shared history intervals as their simulated-turn source for actor turn rows/traces while preserving the existing content-editor-facing report shape; the current scenario recorder remains legacy PNG/GIF fallback tooling, with a future history playback / SadConsole-rendered export preferred over deeper recorder migration.
- entity panel projection is available in Content as a first-pass frontend-neutral DTO over `EntityInspectionPanel`, containment breadcrumbs, action state, inventory grids, local turn-order content rows, optional action-plan / experimental behavior-provider summary, and structured local log snippets; rich layout/focus/rendering remains frontend-owned and the DTO may evolve with SadConsole feedback.
...
- experimental runtime behavior-provider overrides are available in Core/SadConsole as spike-only simulation state: an actor can be assigned a provider entity whose action plan is resolved with the actor as body/action-state owner, while the provider is skipped by the current turn scheduler. Core controlled commands `GiveOverwrite` and `TakeOverwrite` plus SadConsole `G`/`T` debug prompt flows can assign/clear this relation at runtime. The current spike simplification keeps the provider's ordinary entity location in inventory while the overwrite relation represents the abstract slot. This has no descriptor/YAML/editor-service authoring support yet and should not be treated as content-facing support until promoted.
...
2. exercise Gate 2 target acquisition and greedy seeking content using `AcquireNearestTarget` and `SeekTarget`;
3. reassess whether the next batch should prioritize `Wait`-style behavior control, richer target filters, or deferred transfer/location primitives.
