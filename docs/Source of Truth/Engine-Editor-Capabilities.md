...
- merged inventory topology layers are available as a prototype Core/Content/editor capability: authored `mergedLayers` compose one or more entity-owned inventory spaces into a durable navigable layer by explicit contribution origins, with optional cardinal edge-to-edge `seams` for same-index non-Euclidean links such as wraps and rotations. Content validation resolves owner entity IDs from authored carried layouts, rejects overlaps, disconnected layers, duplicate/multi-layer owner participation, unusable/missing owners, seam edge-length mismatches, non-cardinal endpoints, and directional conflicts before materialization. Editor services, agent snapshots/operations, frontend editor summaries, YAML round-tripping, scenario materialization, and Beta manual showcase scenarios can author/inspect the prototype shape. Polished frontend visualization, generated corridors/cells, edge-join shorthand, and final stable authoring UX remain follow-up.
