...
- preferred template `targeting` profiles for pre-plan target-slot selection by shared actor Octagonal-distance range, per-rule optional target template, target-capability adjectives, and Core-owned locality origins (`CurrentPlace`, `OwnInventory`, `PeerInventories`), with legacy `targetingRules` still loadable;
