...
- entity templates and presentations, including semantic `presentationId` / `paletteId` authoring with legacy glyph/color fallback compatibility and presentation-only entity `material` metadata (`metal`, `wood`, `stone`, or undefined/debug fallback);
...
- preferred template `targeting` profiles for pre-plan target-slot selection by shared actor Octagonal-distance range, per-rule optional target template, target-capability adjectives, and Core-owned locality origins (`CurrentPlace`, `OwnInventory`, `PeerInventories`, `ContainingEntityCurrentPlace`), with legacy `targetingRules` still loadable;
...
- canonical ordered behavior-chain descriptors with promoted `Move`, `TransformAdjacentToInventory`, `TransformInventoryToAdjacent`, `Transfer`, `EnterTarget`, `ExitFacing`, `Push`, `CreateEntity`, `PolymorphTarget`, and `DestroyTarget` plus prototype-compatible `MoveFacing`, `Backstep`, `PickupTarget`, `DropFacing`, `PushFacing`, `TargetPathMove`, `GiveTarget`, `TakeTarget`, and `ApplyPrePlan`/`ApplyMainPlan`/`ApplyPostPlan` Action Steps have Core runtime, Action Step catalog metadata, descriptor/YAML, hardened validation/default handling, editor service, agent API, and frontend/editor service support that makes canonical chains visually primary over legacy low-level authoring. `CreateEntity`, `PolymorphTarget`, and `DestroyTarget` are canonical lifecycle/state-alteration steps but are not yet polished as player-controlled workflows; they are best suited to computer-controlled actors until gameplay needs promote richer player-facing UX. Runtime compatibility remains for legacy metadata-setting steps `TurnLeft`, `TurnRight`, `ReverseFacing`, `CreateFacing`, and the retired coordinate-targeting helpers `AcquireNearestTarget`, `SeekTarget`, `FleeTarget`, `MaintainChebyshevDistanceTwo`, `StrafeClockwise`, and `StrafeAnticlockwise`, but new canonical authoring hides/rejects or discourages them in favor of targeting profiles and `TargetPathMove`.
...
- shared Core topology and adjacency evaluation are graph-materialized for default grid movement/adjacency facts plus experimental entity-authored inventory-boundary topology policy and first-slice merged-inventory source-cell links. `TopologyGraphMaterializer` emits eight-way default-grid edges, source-cell-link seams, composed intercardinal source-cell-link seam edges, and entity topology policy edges; `MovementService` consumes the materialized graph directly for adjacent relocation, graph-native movement-edge results, and compatibility adjacency/destination APIs. Plain default-grid adjacency includes cardinal and intercardinal neighbors, and rejects intercardinal adjacency when both orthogonal corner spaces are occupied by entities; composed seam diagonals follow the same rule when one orthogonal leg is an explicit source-cell link. Direct action internals and canonical inventory primitives preserve graph adjacency facts in traces for pickup, enter, and transfer-counterparty resolution while continuing to expose coordinates as projection metadata. Target-path movement pathing and distance-band behavior use graph traversal and can cross source-cell-link topology between planes; orbit angular ordering still intentionally consumes coordinate projection geometry as an embedding question. `EntityTopologyPolicy` is authorable in YAML, editor snapshots/mutations, and agent APIs as `None`, `ConnectsInward`, `ConnectsOutward`, or `ConnectsInwardAndOutward`; it creates directed topology adjacency between an entity's exterior adjacent cells and preferred/matching cells on the edge of that entity's inventory plane. Merged inventory layers can now author YAML `joins` with cardinal owner edges and `align: Center`; Content validation resolves those joins to source-cell links, rejects directional conflicts, and scenario materialization populates `WorldState.SourceCellLinks` for movement over explicit room-to-hall seams. Editable documents, `ContentEditorService`, `AgentContentEditorApi`, and frontend editor snapshots preserve/list the join shape through shared Content services. Topology policy and source-cell links currently support movement and shared topology consumers; general portals, entanglement relations, final vision semantics, and polished frontend visualization remain planned/deferred.
- current merged-layer topology rule: merged-layer `spaces[].origin` values are projection/layout metadata only and do not create cross-contributor runtime adjacency by coordinate contact or overlap. Runtime movement, POV, pathing, and adjacency cross contributors only through explicit topology such as materialized source-cell links from authored joins. Same-contributor cells retain ordinary same-plane grid adjacency.
- runtime control-source / Action Choice support is available in Core for canonical `Move`, Pickup/Drop, typed Enter/Exit, canonical Transfer, and canonical Push prompts: `EntityActionState` stores `Automatic` or `PlayerChoice` control source and clones/restores it through `WorldState`; persisted scenario `PlayerControls` initialize controlled entities to `PlayerChoice`; `ActionChoiceService` returns a Core-owned request for `PlayerChoice` actors with a canonical `Move` step, coalescing multiple authored `Move` steps into one eight-direction absolute movement choice; authored `TransformAdjacentToInventory`/`PickupTarget` steps expose adjacent pickup targets plus inventory-slot destinations from Core affordance facts; authored `TransformInventoryToAdjacent`/`DropFacing` steps expose carried sources plus adjacent map destinations for the first canonical Drop selection path; authored `EnterTarget` exposes Enter target choices; authored `ExitFacing` exposes Exit direction choices; authored `Transfer` exposes adjacent counterparties and item choices from either inventory, deriving ActorToTarget or TargetToActor from the selected item's owner; authored `Push` exposes adjacent push targets and per-target target-relative direction choices; submitted Move/Pickup/Drop/Enter/Exit/Transfer/Push choices execute through shared controlled-command/turn/history semantics; and a narrow Core-owned authored-step fallback can submit non-parameterized selected Action Steps through `ActionChoiceService`/`SimulationHistorySession` instead of frontend-owned interpreter calls while those steps await richer typed choice DTOs. Full pre/main/post descriptor composition, richer selected-destination Enter prompts, and broader workflow-descriptor polish remain planned follow-up.
- initiative-aware `PlayerChoice` stepping is available in Core, Content/headless runs, and frontend play mode: shared stepping advances automatic actors in deterministic scenario initiative order until a `PlayerChoice` actor is reached, returns that actor's Action Choice request or a structured diagnostic when no request can be built, and supports multiple player-controlled starts and playerless automatic advancement. SadConsole no longer treats the scenario player entity as always first for the prompt loop. Headless persisted scenario runs no longer auto-resolve `PlayerChoice` actors by default; they record automatic intervals before the prompt and report the pending prompt as a runtime observation. For content debugging only, `AgentContentEditorApi` and content tools can run a persisted scenario with `ignorePlayerChoiceControl: true` so controlled actors execute authored/default plans automatically. Core `SimulationHistorySession` can retarget the current controlled actor without restarting history, preserving intervals and rollback state, and SadConsole uses that retargeting path so play-mode history/log continuity survives active controlled-actor changes.
...
- Template material is included in frontend editor snapshots and can be set/cleared through shared editor metadata mutations and `AgentContentEditorApi`; supported tool schema values are `metal`, `wood`, and `stone`, with clear/omitted material meaning undefined debug/fallback presentation.
...
- Content service/API action-step mutations also support target-slot/self-target updates, typed options for `directionMode`, `transferDirection`, apply-plan `planId`, `CreateEntity`/`PolymorphTarget` template IDs, create placement, `TargetPathMove` path mode/distance/orbit fields, and optional action-step costs. These fields are validated through shared Content services and projected in previews/snapshots rather than edited by ad-hoc YAML mutation.
...
| `TransformAdjacentToInventory` / `PickupTarget` | Promoted canonical pickup semantics; `PickupTarget` remains compatibility name and primitive-backed linked plan support remains transitional | `Target` | `Self` | Reads persistent actor `Target`, attempts pickup into the first available valid actor inventory coordinate using deterministic row-major order (`Y`, then `X`) across the actor's inventory plane, and falls through to the next ordered Action Step when pickup fails. Pickup requires the target bulk to fit the actor aperture and the actor bulk to exceed the target aperture, preventing same-actor enterable/portable overlap. Action Choice exposes adjacent pickup targets plus inventory destinations for player-controlled actors; submitted choices execute through shared controlled-command/history semantics. Prefer `TransformAdjacentToInventory` for new canonical authoring. |
| `TransformInventoryToAdjacent` / `DropFacing` | Promoted canonical drop semantics; `DropFacing` remains compatibility name | `Facing` | `West` | Drops a carried entity from actor inventory to an adjacent map destination. The compatibility behavior-chain step drops the first carried entity in the actor's `Facing` direction; Drop requires the carried entity bulk to fit source/container aperture rules and the actor bulk to exceed the carried entity aperture. The Action Choice seam exposes carried sources plus adjacent destinations for player-controlled actors and submits through shared controlled-command/history semantics. Prefer `TransformInventoryToAdjacent` for new canonical authoring. |
| `Transfer` | Promoted canonical peer inventory Action Step with descriptor/YAML/editor/agent/Action Choice/SadConsole support | moving entity via `targetLabel` preferred or `targetSlot`; adjacent counterparty via optional `counterpartyTargetLabel`/`counterpartyTargetSlot` or compatibility `directionMode`; `transferDirection` | `Target=Self` for target-slot defaulting where current workflows default targets; authors should set target label/rule and either counterparty target or direction explicitly | Atomically transfers a selected concrete moving entity between actor and adjacent counterparty inventories. `ActorToTarget` is give-like and checks the adjacent destination holder's `EnterPolicy`/aperture/capacity while ignoring actor `ExitPolicy`; `TargetToActor` is take-like and checks the adjacent source holder's `ExitPolicy`/aperture while ignoring actor `EnterPolicy`. Autonomous plans may target the counterparty entity directly; direction-based transfer remains backwards-compatible. Player Action Choice chooses counterparty then item and derives direction from the selected owner. Legacy `GiveTarget`/`TakeTarget` remain compatibility/prototype shortcuts. |
| `Push` | Promoted canonical Action Step at Core/descriptor/YAML/validation/editor service/agent API/Action Choice/new frontend layers | `Target`; `directionMode` | `Target=Self` for target-slot defaulting where current workflows default targets; authors should set target label/rule and direction explicitly | Forces the selected adjacent target to move one adjacent step in the target-relative `directionMode` when the target bulk fits actor aperture and the target destination is legal/open. The actor does not move. Action Choice exposes push targets plus per-target direction options and submissions execute through shared controlled-command/history semantics. Legacy `PushFacing` remains compatibility behavior with blocker-forward plus actor-follow-through semantics. A dedicated player-facing Log component remains later frontend work even though structured outcomes/history rows exist. |
...
| `DestroyTarget` | Canonical lifecycle/state-alteration step; computer-controlled use is supported, player-facing workflow polish remains follow-up | `Target` | `Self` | Recursively destroys persistent actor `Target`, including its inventory space and contained entities. The current first pass rejects self-destruction. Useful for autonomous lifecycle, consumption, cleanup, and one-shot state changes. |
| `CreateFacing` | Legacy/prototype placeholder spawner; not recommended for new content | `Facing` | `West` | Creates a placeholder rock-like entity in the actor's `Facing` direction when the destination is valid/open. Prefer template-backed `CreateEntity` for new spawning/reproduction content because `CreateFacing` does not carry authored template/presentation identity. |
| `CreateEntity` | Canonical template-backed lifecycle/spawning step; computer-controlled use is supported, player-facing workflow polish remains follow-up | `templateId`; optional `createPlacement`; `directionMode` when placement is `Facing` | none, except placement fields authors set explicitly | Creates a runtime entity from an authored template in the first open adjacent cell by default, or in a resolved facing direction when `createPlacement: Facing` is authored. New entities receive template name, inventory dimensions, bulk, aperture, policies, topology policy, default action plan, initial facing, runtime template identity for content/presentation lookup, and an inventory plane when applicable. Optional costs are consumed only after successful creation. Useful for autonomous spawning/reproduction and lifecycle output. |
| `PolymorphTarget` | Canonical template-backed lifecycle/state-alteration step; computer-controlled use is supported, player-facing workflow polish remains follow-up | `Target` or `targetSelf`; `templateId` | `Target=Self` for target-slot defaulting where current workflows default targets | Changes the selected entity to another authored template while preserving runtime entity ID, current location, current inventory contents, and existing facing. It applies the new template name/bulk/aperture/policies/topology policy/default action plan/runtime template identity. Optional costs are consumed only after successful polymorph. Useful for autonomous phase changes, transformation, and lifecycle state changes. |
| `TargetPathMove` | Prototype-compatible graph-native target-path movement step | `Target`; `pathMode`; `desiredDistance` for MaintainDistance/Orbit; `orbitDirection` for Orbit | `Target=Self` for target-slot defaulting where current workflows default targets | Moves one step using legal topology relative to the target and can cross source-cell-link topology. `SeekAdjacency` and `FleeAdjacency` move relative to target adjacency; `MaintainDistance` and `Orbit` use authored integer distance bands where diagonal movement is more expensive than cardinal movement, producing octagonal rings. Successful movement preserves `Target` and sets `Facing` to the actual direction moved. Orbit angular ordering still consumes coordinate projection geometry. Prefer this over retired coordinate target-movement helpers for new graph-first content. |
...
| `AcquireNearestTarget` | Retired legacy metadata-setting step; new canonical authoring should use template `targeting` profiles | none | none | Runtime rejects this retired step in new graph-first behavior-chain execution. Use template targeting profiles/rules with stable labels so target refresh is read-only, authorable, and previewable before action-plan resolution. |
| `SeekTarget` / `FleeTarget` / `MaintainChebyshevDistanceTwo` / `StrafeClockwise` / `StrafeAnticlockwise` | Retired legacy coordinate target-movement helpers | `Target` | `Self` | Runtime rejects these retired helpers in new graph-first behavior-chain execution. Use `TargetPathMove` with `pathMode` instead so movement uses legal topology and source-cell links. |
...
| `EnterTarget` | Promoted canonical enter semantics with shared controlled-command/affordance, Action Choice, POV adjective, and content fixture support | `Target` | `Self` | Reads persistent actor `Target`, verifies the target exists, is not self, is adjacent/reachable by topology, and has usable inventory, then moves the actor into the coordinate selected by the target owner's `EnterPolicy` or compatibility row-major default. Successful enter consumes the turn. Missing/invalid/self/non-adjacent target, target without usable inventory, no space, policy rejection, or aperture failure fails/falls through. The constrained move checks the destination inventory owner's aperture; target-inventory failures use target-centric trace reasons/details. |
| `ExitFacing` | Promoted canonical exit semantics with shared controlled-command/affordance, Action Choice, and content fixture support | `Facing` | `West` | Reads persistent actor `Facing`, finds the entity whose inventory plane currently contains the actor, and moves the actor to the topology destination adjacent to that container in `Facing`, respecting the source/container owner's `ExitPolicy` or compatibility default. Successful exit consumes the turn. Not being in an entity inventory, blocked/out-of-bounds destination, missing container, policy rejection, or aperture failure fails/falls through. The constrained move checks the source/container inventory owner's aperture before placing outside it. |
...
Canonical behavior-chain traces can be summarized through `BehaviorChainTraceFormatter`. The compact formatter reports the root plan outcome, each attempted Action Step, success/failure reason, whether fallback continued or stopped, canonical state reads/writes such as `Facing` and `Target`, Backstep preserve-facing results, Transfer and GiveTarget/TakeTarget transfer details, EnterTarget/ExitFacing containment movement details, Push target/direction details, template-backed CreateEntity/PolymorphTarget lifecycle details where available, TargetPathMove path-mode/distance/orbit movement details, cost reads/consumption/failures, and the terminal consumed-turn outcome. Retired metadata-setting and coordinate target-movement helpers remain historical trace references only.

Graph-first target movement authoring should combine template `targeting` profiles with `TargetPathMove`. The targeting profile refreshes a labeled target before plan resolution; the Action Step consumes that target label and path parameters:
  ...
  rat:
    targeting:
      ...
      rules:
        - slot: 1
          label: food
          targetTemplateId: cheese
    defaultActionPlan: ratSeekFood
actionPlans:
  ratSeekFood:
    id: ratSeekFood
    behavior:
...
Maintained/orbiting distance bands author `desiredDistance` and, for orbit, `orbitDirection`:
...
Retired helpers such as `AcquireNearestTarget`, `SeekTarget`, `FleeTarget`, `MaintainChebyshevDistanceTwo`, `StrafeClockwise`, and `StrafeAnticlockwise` should not be used for new content. Existing legacy references should migrate to template targeting profiles plus `TargetPathMove` when touched.
...
| Targeting rules | Stable | Yes | Yes | Yes | Yes | Yes | Display | Preferred template `targeting` profiles select nearest candidates before plan evaluation using shared actor Octagonal-distance `range`, rule label/slot as verb, optional target template as noun, `targetCapabilities` as adjectives, and rule/default locality origins for where the actor looks. Supported locality origins are `CurrentPlace`, `OwnInventory`, `PeerInventories`, and `ContainingEntityCurrentPlace`; the containing-entity origin applies only while the actor is inside an entity inventory and considers the containing entity plus peers in that containing entity's current place, with distances measured from the containing entity's current-place coordinate. Legacy `targetingRules[].range` remains loadable for compatibility. Supported target capabilities are the non-mutating target-affordance Action Steps `TransformAdjacentToInventory`/`PickupTarget`, `EnterTarget`, `GiveTarget`, `TakeTarget`, `DestroyTarget`, `Push`, and compatibility `PushFacing`; prefer `Push` for new canonical pushability adjectives. Shared candidate preview reports matching candidates/provenance and Octagonal distances without mutating target slots. Behavior steps should prefer authored `targetLabel`, may use explicit `targetSelf` when the actor is the target, and retain numeric `targetSlot` for compatibility/advanced use. Current targeting selects one nearest candidate rather than target sets; count/density predicates are planned follow-up. Frontend editor snapshots and SadConsole authoring display profile source, shared/default locality, rule locality, effective locality, and self target references. |
...
- prefer typed commands for canonical ordered behavior-chain Action Steps for normal movement, pickup, drop, enter, exit, transfer, push, lifecycle/state alteration (`CreateEntity`, `PolymorphTarget`, `DestroyTarget`), costs, targeting profiles, and graph-native `TargetPathMove` authoring;
...
2. expand targeting/awareness from single nearest Octagonal-distance candidate selection to richer target sets/counts/density predicates when ecology or faction experiments need them;
