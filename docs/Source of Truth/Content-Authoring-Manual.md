...
For new composed/exhibit scenarios, treat the scenario root as a world/level container rather than a gameplay room. In persisted scenarios whose root stages non-actor top-level places, the scenario-root inventory plane is a runtime staging plane: the root still instantiates and can hold rooms, halls, merged-layer contributors, and setup structure, but ordinary movement/pathing/Exit affordances do not treat the root plane as a player-facing gameplay location. Players and ordinary interactable objects should normally be placed inside room-like child entities contained by the root, with playable actors authored as placed instances using `controller: Player`. Use direct placement in the scenario-root plane only for root-only compatibility tests, small engine/editor fixtures, intentionally container-like scenarios, or legacy content awaiting migration.
...
| Player start coordinate | Nullable legacy fallback placement. For structural roots, a start targeting a usable top-level place is migrated into that place with a setup warning; empty root-plane legacy starts keep root-plane compatibility for existing content and should be migrated to placed `controller: Player`. Missing start plus no placed `controller: Player` produces a playerless scenario rather than implicit `(0,0)` insertion. |
