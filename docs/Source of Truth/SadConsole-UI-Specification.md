...
- Consumer Play mode now has a completed Actor POV MVP layout foundation over `ActorPovPlayProjection`: `ActorPovPlayLayoutResolver`, `ActorPovPlayScreenModel`, and `ActorPovPlayComponentFactory` resolve named regions for parent chain, current POV, world inspection, actor inventory, carried-item inspection, connector/chrome layers, and diagnostics. The MVP renders the parent chain as compact text rows, current/inspection/actor inventory spaces as region-local inventory-space components, and debug-only Actor POV diagnostics chrome.
...
|---|---|---|---|---|---|
| UI-M01 | Define layered screen regions. Example: component 1 takes the full screen on bottom/layer 0; component 2 draws on layer 2 on the left 30%; component 3 draws on layer 2 in 10% of remaining area. Main-game Play mode should use this to compose inventory-space nodes, connector lines, prompts, debug overlays, and future collapsed/pinned branch controls. | Strong fit. Layering, clipping, bounds, focus, and hit-testing are frontend-owned presentation state. The containment-tree view is presentation state over shared containment and inventory facts, not a new simulation model. | SadConsole's `ScreenObject` hierarchy is designed for nested screen objects. Child objects render through the root screen automatically. Children render in collection order by default; `SortOrder` can be used with `Children.Sort()` for depth control. `LayeredScreenSurface` supports stacked full-surface cell layers when independent glyph layers on one surface are needed. | `LayeredScreenSurface` layers are full-surface layers, not a complete relative-region layout system. Relative region definitions and z-layer policy should be project-owned screen-model logic, resolved into SadConsole child object positions/sizes. Need define stable project rules for layer ordering, clipping, overlap, input priority, and whether connector lines draw below or above inventory-space glyphs. | Initial mock play-mode implementation complete through `GameplayMockLayout` named/layered regions; broader first-class layout engine remains future work. Containment-tree Play mode should reuse the same layer policy rather than adding ad hoc surfaces. |
| UI-M02 | Define regions by relative screen placement, not pixels or tiles. The containment-tree layout should resolve the scenario-root/current-place node, inspected child nodes, breadcrumb-chain nodes, shared-root branches, and eventually expanded/collapsed tree branches from relative rules into concrete cell bounds. | Strong fit if relative definitions resolve into concrete cell rectangles at render time. The durable spec should remain relative; only the renderer receives cell bounds. | SadConsole positions `ScreenObject`s relative to parents and exposes absolute positions after hierarchy resolution. Nested `ScreenObject` containers are suitable for recursive layout. | SadConsole does not appear to provide a general CSS/Flex/Grid-style relative layout manager. We should build a pure tested project resolver for percentages, ratios, anchors, gaps, min/max cells, and rounding. Full arbitrary trees may exceed the viewport, so the resolver must support clipping, scrolling, collapsed nodes, or partial branch selection before the UI promises always-visible whole-scenario display. | Initial mock play-mode ratio split complete for HUD/content and current-place/inspection; generalized recursive resolver remains future work. The next accepted resolver use should support at least current-space plus one linked inspected child-space. |
...
| UI-M04 | Main-game Play mode presents containment as linked inventory-space nodes. The expected final UX is a scenario-root or current-root inventory space at a stable anchor, with each visible contained entity that owns a usable inventory space connected by a line from the parent entity's rendered cell to the child inventory-space node. The same model recurses for visible child nodes. | Strong fit. Existing `ActorPovPlayProjection`, `EntityPanelProjectionService`, `EntityContainmentPathService`, point-of-view projection, and `InventorySpaceViewModel` provide most node facts. Frontend state owns which nodes are visible, expanded, collapsed, pinned, focused, selected, panned, scaled, or clipped. | SadConsole can render each inventory space as an ordinary component and draw connector lines through a separate overlay pass. The component gallery already proves a `DrawCallCustom` connector-line spike with endpoints derived from SadConsole cells. | A full arbitrary scenario tree may not fit on screen. The durable UX should support partial views first: current-space plus inspected child-space, breadcrumb chains, compact/stacked parent inventory nodes, then shared-root branches, then experimental full-tree/collapsed-tree navigation. Traversal must stay cycle-safe and should preserve shared-service diagnostics instead of guessing ancestry in the frontend. | Actor POV MVP foundation complete: named regions and projection-backed components render current POV, parent-chain scaffold, world inspection, actor inventory, carried inspection, and debug diagnostics. Next target is parent inventory nodes plus connector endpoints across regions. |
...
| UI-N03 | Draw components on top of region definitions and reposition them during gameplay; e.g. floating popup modals. In containment-tree Play mode, prompts and debug overlays should float above the linked-space canvas without breaking node placement or connector geometry. | Strong fit. Existing project renderer already has a centered overlay child console pattern; FED-016 accepts child `Console` overlays. | SadConsole `Window` provides bordered/titled draggable panels, modal display via `Show(true)`, close-on-Escape, and built-in controls. `IgnoreParentPosition` supports screen-fixed overlays. Child objects plus `SortOrder` support non-window custom overlays. | For game-styled overlays, project components may still be preferable to SadConsole `Window` controls to preserve existing theme/glyph vocabulary. Debug overlays should draw over all normal Play content when enabled. Semi-transparent debug overlays are desirable but deferred; opaque topmost debug glyphs/text are acceptable for the current Play-mode MVP. Need decide when to use SadConsole `Window` versus project `IUiComponent` rendered on child consoles. Connector lines should not be implemented as prompt overlays; they are part of the linked-space canvas. | Promote generalized overlay/floating/debug-region support into layout engine and gallery after linked-space node and connector layers have a stable z-order. |
| UI-N04 | Define components themselves into internal regions; e.g. component 6.5 divides horizontally into 30:70 regardless of current component dimensions. Containment-tree nodes may need internal regions for title/chrome, visible grid viewport, collapsed child summaries, branch controls, and local activity summaries. | Strong fit and should be designed into the same resolver recursively rather than reinvented per component. | SadConsole `ScreenObject` containers can nest at unlimited depth, with positions relative to parent. Parent/child layout naturally maps to component-internal regions. | Same limitation as UI-M02: SadConsole provides composition primitives, not a full relative layout language. Need project-owned recursive layout definitions and resolved region tree. Node-internal regions must keep inventory cell geometry stable enough for connector endpoints and hit-testing. | Include in resolver design from the start, even if only used after top-level migration. First linked-space slices can keep node internals simple by reusing `InventorySpaceComponent`. |
| UI-N05 | Support mouse hit-testing and click interaction over resolved screen regions and cells, including click-to-focus, click-to-inspect, click-to-expand/collapse containment nodes, and click-to-select prompt targets where equivalent keyboard actions exist. | Fit as frontend-owned input, focus, hover, hit-testing, and selection behavior. It must submit through shared runtime/editor services and must not invent legality. | SadConsole supports mouse input on screen objects, exclusive mouse routing, focus events, relative/absolute positioning, and child-object hierarchy. A resolved region tree gives the project a natural hit-test index. | Overlapping layers require deterministic topmost-hit rules. Cell hit-testing must account for render scaling, pixel margins, and any `UsePixelPositioning` surfaces. Mouse behavior should remain a convenience path over the directional/Select/Cancel model. Pixel connector lines should be visual affordances, not the primary hit target, unless a later explicit connector hit-test model is added. | Pure topmost-region hit-test and live hover diagnostics complete for mock play mode; click-to-act/focus remains future work. Containment-tree work should first expose node/cell hit-test facts before adding mouse expansion. |
| UI-N06 | Support collapsible, expandable, pinned, and focusable multi-panel layouts as presentation state over the same region system. This is required for practical containment-tree navigation once more than one chain or branch is visible. | Fit as frontend-owned component state and layout policy. Useful for entity panel chains, editor panels, dense local activity, inspection surfaces, and containment-tree branches. | SadConsole child visibility, enabled state, and component focus hooks support hiding/collapsing and focus treatment. Existing project component focus/border patterns and panel-chain tests provide a foundation. | Collapse/pin state should not change underlying runtime/editor facts. Need min-size, overflow, and selection rules so collapsed panels remain discoverable and keyboard-operable. Collapsed child nodes should still preserve enough parent-to-child relationship information to maintain the containment mental model. | Related but distinct from UI-M01/UI-M02; model as region/component state atop the resolver. This becomes mandatory before promising whole-scenario tree browsing. |
...
- **Linked containment-tree Play presentation** maps directly to UI-M04, with UI-M01/UI-M02 providing node and connector layout mechanics.
...
6. **Connector lines need a reusable overlay contract.** The component gallery proves `DrawCallCustom` can draw smooth lines from cell-derived endpoints, but Play mode still needs a stable model for endpoint geometry, z-order, redraw timing, resizing, and fallback tile-line rendering before connectors become canonical UI.
7. **Whole-scenario containment trees may be too large.** The expected main-game presentation is linked containment-space nodes, but the durable interaction model should not promise every scenario is fully visible at once. Chains, branches, collapsed nodes, pinned nodes, scrolling, clipping, and navigable partial trees are acceptable presentations over the same facts.
...
Completed Actor POV MVP layout foundation sprint: `docs/Archived/Actor-POV-Play-Layout-Foundation-Sprint-Plan.md` established the current Consumer Play region resolver, projection-backed screen model, region-local components for current/world/actor/carried spaces, parent-chain scaffold, and debug diagnostics chrome. It intentionally stopped before parent inventory nodes, connector endpoints, per-node scale/pan controls, and reusable region separator chrome.

Containment-tree rollout sequence:

1. Promote parent chain entries from compact text rows into vertically stacked compact inventory-space nodes where each node can expose rendered cell geometry. Use `ActorPovPlayProjection.ParentChain` and preserve `ChildCoordinateInEntityInventory` for connector endpoints.
2. Promote cross-region connector endpoints for Actor POV: parent-chain node cell -> current POV region, current POV candidate cell -> world inspection region, and actor-inventory cell -> carried inspection region. Reuse `ConnectorLineViewModel`/`ConnectorLineDrawCallRenderer`; endpoints should derive from rendered component/cell geometry after layout and viewport resolution.
3. Add per-node render profiles and viewport state application. Each visible inventory space should be able to use compact/current/actor/debug profiles and independent pan/crop state without changing runtime facts.
4. Add reusable region chrome/separator components for Actor POV regions: divider borders, optional labels, metadata/status rows, omitted counts, and debug-only diagnostics. Keep chrome theme/component-owned rather than shell-owned drawing.
5. Add explicit inspection selection controls over projected world and carried candidates. Selection remains frontend state unless inspectability rules require shared projection changes.
6. Add shared-root branching for a second visible chain. Use shared-root containment facts rather than frontend ancestry guesses, and preserve incomplete/cycle diagnostics. Covers UI-M04 and prepares UI-N06 for branch focus/pin state.
7. Experiment with broader scenario-root tree display over the same linked-space model: expanded/collapsed child nodes, pinned branches, scrolling or clipping, and navigable partial-tree views. Do not promise arbitrary full-tree fit until layout evidence supports it. Covers UI-M02, UI-M04, UI-N04, UI-N05, and UI-N06.
8. After linked-space layout, connector z-order, and focus routing are stable, resume broader layout infrastructure work: generalized recursive relative resolver, deterministic z-layer ordering, display-metrics spike, manual recalculation, responsive resize reassessment, and optional user layout override/profile support. Covers UI-M01, UI-M02, UI-M03, UI-N01, UI-N02, UI-N03, and UI-N04.
