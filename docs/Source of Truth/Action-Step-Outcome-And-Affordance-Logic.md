...
| `TransformAdjacentToInventory` / `PickupTarget` | `Target`; usable actor inventory with space | Target exists, is not actor, fits actor aperture, and actor bulk exceeds target aperture | Target is in an adjacent pickup-valid peer/world location for the actor | Target moves into first valid actor inventory coordinate in row-major order; turn consumed | Missing/self/invalid/non-adjacent target, no usable actor inventory, no space, target too bulky, or actor not larger than target aperture falls through. |
| `TransformInventoryToAdjacent` / `DropFacing` | `Facing`; actor carries at least one entity | First carried entity fits source/container aperture rules and actor bulk exceeds carried entity aperture | Destination one cell in `Facing` on actor plane is valid/open | First carried entity drops to facing cell; turn consumed | Nothing carried, invalid/blocked destination, target too bulky, actor not larger than target aperture, or aperture failure falls through. |
| `Transfer` | Moving entity selected by `targetLabel`/`targetSlot`; adjacent counterparty direction from `directionMode`; `transferDirection` | For `ActorToTarget`, moving entity is actor-carried and counterparty has usable inventory/space; for `TargetToActor`, moving entity is counterparty-carried and actor has usable inventory/space | Counterparty is adjacent to actor in the resolved direction | `ActorToTarget` atomically moves the selected entity into the first valid counterparty inventory coordinate selected by counterparty `EnterPolicy`; `TargetToActor` atomically moves the selected entity into the first valid actor inventory coordinate; turn consumed | Missing selected moving entity, missing/non-adjacent/invalid counterparty, wrong owner for direction, no usable inventory/space, destination `EnterPolicy` failure for ActorToTarget, source `ExitPolicy` failure for TargetToActor, or aperture failure falls through without intermediate world placement. Actor-owned policies do not constrain the actor's current transfer. |
| `Push` | `Target`; `directionMode` | Target exists, is not actor, is adjacent/reachable, and target bulk fits actor aperture | Target-relative destination in `directionMode` is valid/open | Target moves one adjacent step in the selected direction; actor does not move; turn consumed | Missing/self/non-adjacent target, target too bulky for actor aperture, unresolved direction, blocked destination, out-of-bounds, or invalid movement falls through. |
...
| `GiveTarget` | `Target`; actor carries at least one entity | Target exists, is not actor, has usable inventory with space; carried item fits both apertures and source/destination policies | No adjacency requirement beyond target validity/current state | First actor-carried entity transfers into the first valid target inventory coordinate selected by target `EnterPolicy`; turn consumed | Missing/self/invalid target, nothing carried, no usable target inventory, no space, source `ExitPolicy`, destination `EnterPolicy`, or source/destination aperture failure falls through. |
| `TakeTarget` | `Target`; actor has usable inventory with space | Target exists, is not actor, has usable inventory and at least one carried entity; taken item fits apertures and policies | No adjacency requirement beyond target validity/current state | First target-carried entity transfers into the first valid actor inventory coordinate selected by actor `EnterPolicy`; turn consumed | Missing/self/invalid target, empty/no usable target inventory, no usable actor inventory, no space, source `ExitPolicy`, destination `EnterPolicy`, or aperture failure falls through. |
| `EnterTarget` | `Target`; actor bulk fits target aperture | Target exists, is not actor or contained descendant, has usable inventory with space, and destination `EnterPolicy` can select a valid coordinate | Actor is adjacent to target on same plane | Actor moves into target inventory at the coordinate selected by target `EnterPolicy`; turn consumed | Missing/self/contained-descendant/non-adjacent target, no usable target inventory, no space, destination owner aperture failure, or policy failure falls through. |
| `ExitFacing` | `Facing`; actor bulk fits current container aperture and source `ExitPolicy` allows selected direction | Current inventory owner/container exists | Actor is currently in an entity inventory; destination adjacent to container in `Facing` is valid/open | Actor moves out to adjacent container cell; turn consumed | Not contained, missing container, blocked/out-of-bounds destination, source/container aperture failure, or source `ExitPolicy` failure falls through. |
...
| Actor has usable inventory, aperture admits actee bulk, and actor bulk exceeds actee aperture | Actee is a peer/world entity | Adjacent to actor | pickup / transform adjacent to inventory | Controlled affordance exposes adjacent pickup sources and row-major actor-inventory destinations. Preferred canonical name is `TransformAdjacentToInventory`; compatibility `PickupTarget` uses the same current `Target` semantics. This prevents an actee from being both enterable and portable by the same actor. |
...
| Actor carries a selected moving entity or adjacent counterparty carries a selected moving entity | Counterparty has usable inventory/space for ActorToTarget, or actor has usable inventory/space for TargetToActor | Counterparty is adjacent to actor | transfer, give, take | Canonical `Transfer` is the preferred peer inventory verb. Player Action Choice chooses the counterparty, then an item from either inventory; Core derives give-like ActorToTarget or take-like TargetToActor from item ownership. Authored autonomous Transfer declares direction and selected moving entity. |
| Actor has plane position and aperture admits actee bulk | Actee is an adjacent/reachable peer/world entity | Actee destination in selected actee-relative direction is open | push / forced target movement | Canonical `Push` moves the actee only; legacy `PushFacing` remains blocker-forward plus actor-follow-through compatibility behavior. |
...
- Controlled direct-player affordances currently cover `move`, `pickup`, `drop`, `enter`, `exit`, and `transfer`. Other verbs are behavior-chain Action Steps and may appear in previews/traces rather than direct-player command affordances.
...
- Constrained inventory verbs (`PickupTarget`, `DropFacing`, `Transfer`, `GiveTarget`, `TakeTarget`, `EnterTarget`, `ExitFacing`) enforce Bulk/Aperture transition rules. `Teleport` remains the advanced unconstrained relocation primitive and is outside the canonical verb table above.
