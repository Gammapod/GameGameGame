...
| UI-M04 | Main-game Play mode presents containment as linked inventory-space nodes. The expected final UX is a scenario-root or current-root inventory space at a stable anchor, with each visible contained entity that owns a usable inventory space connected by a line from the parent entity's rendered cell to the child inventory-space node. The same model recurses for visible child nodes. | Strong fit. Existing `EntityPanelProjectionService`, `EntityContainmentPathService`, point-of-view projection, and `InventorySpaceViewModel` provide most node facts. Frontend state owns which nodes are visible, expanded, collapsed, pinned, focused, or clipped. | SadConsole can render each inventory space as an ordinary component and draw connector lines through a separate overlay pass. The component gallery already proves a `DrawCallCustom` connector-line spike with endpoints derived from SadConsole cells. | A full arbitrary scenario tree may not fit on screen. The durable UX should support partial views first: current-space plus inspected child-space, then breadcrumb chains, then shared-root branches, then experimental full-tree/collapsed-tree navigation. Traversal must stay cycle-safe and should preserve shared-service diagnostics instead of guessing ancestry in the frontend. | New target item. Adopted as the expected main-game presentation direction; implementation should proceed through staged containment-tree slices rather than attempting whole-tree layout first. |
...
2. Add a pure, test-backed linked-space layout model for the smallest useful case: current-place inventory space plus one inspected contained entity's inventory space. The layout should expose parent entity cell anchors, child node bounds, and connector endpoints without changing gameplay behavior. Covers UI-M01, UI-M02, UI-M04, and the stable geometry parts of UI-N05.
3. Replace the separate inspection popup/panel behavior in consumer Play mode with the linked two-space presentation. The inspected entity's inventory space appears beside the current place, connected from the parent entity's rendered cell to the child space. Prompt overlays must continue to float above this linked-space canvas. Covers UI-M04, UI-N03, and existing action-prompt behavior.
4. Generalize from one inspected child to one containment/breadcrumb chain using `EntityContainmentPathService`/point-of-view breadcrumbs. Long chains may reuse existing visible-chain collapse behavior rather than forcing every ancestor on screen. Covers UI-M02, UI-M04, and UI-N06.
5. Add shared-root branching for a second visible chain. Use shared-root containment facts rather than frontend ancestry guesses, and preserve incomplete/cycle diagnostics. Covers UI-M04 and prepares UI-N06 for branch focus/pin state.
6. Experiment with broader scenario-root tree display over the same linked-space model: expanded/collapsed child nodes, pinned branches, scrolling or clipping, and navigable partial-tree views. Do not promise arbitrary full-tree fit until layout evidence supports it. Covers UI-M02, UI-M04, UI-N04, UI-N05, and UI-N06.
7. After linked-space layout, connector z-order, and focus routing are stable, resume broader layout infrastructure work: generalized recursive relative resolver, deterministic z-layer ordering, display-metrics spike, manual recalculation, responsive resize reassessment, and optional user layout override/profile support. Covers UI-M01, UI-M02, UI-M03, UI-N01, UI-N02, UI-N03, and UI-N04.
